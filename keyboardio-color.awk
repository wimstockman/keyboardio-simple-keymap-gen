...
for (k = 1; k <= ncmap; ++k) {
    name = COLORMAP[k];
    left = lefthalf["cmap_"name][1];
    right= righthalf["cmap_"name][1];
    for (i = 2; i <= 32; ++i) {
        left=left OFS lefthalf["cmap_"name][i];
        right=right OFS righthalf["cmap_"name][i];
			kleur = righthalf["cmap_"name][i];
			toets = righthalf[name][i];
			print toets;
			test[kleur][toets];	
        }    
    print  "COLORMAP_"name;
    if (k < ncmap) print "("left OFS right"),";
    else print "("left OFS right")";
    }
print "};"
	for(i in test){
			str="";
			for (j in test[i]){ str=str" || " j }
			
print substr(str,4)"LEDControl.setCrgbAt(r, c, CRGB(0,200,0));"
